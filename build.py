  ...
  <div id="fbarwrap" data-mq="fwrap" style="position:sticky;top:0;z-index:30;background:#f4f1ea;border-bottom:2px solid #17130d">
    <div id="fbar" data-mq="fbar" style="max-width:1200px;box-sizing:border-box;margin:0 auto;padding:12px 30px 13px 30px">
      <div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center">
        ...
        <!-- flex:1 1 150px + max-width:260px, NOT width:260px. A wrapping flex
             row decides line breaks from an item's BASE size, so a 260px box
             jumps to its own line rather than narrowing, however much
             flex-shrink it has. A low basis is the only thing that stops it,
             and this is the whole reason the field wrapped on an iPad.
             The desktop appearance is unchanged, which is why this needs no
             breakpoint: flex-grow resolves BEFORE auto margins consume free
             space, so the field grows to its 260px cap first and margin-left:auto
             then pushes it right with whatever is left. Wide screens look exactly
             as they did; cramped ones narrow instead of wrapping.
             min-width:150px is the floor — below that the field is too small to
             type in, and wrapping is genuinely the better answer. -->
        <input id="q" data-mq="search" type="search" placeholder="Search brand, model, reference…" style="margin-left:auto;flex:1 1 150px;min-width:150px;max-width:260px;padding:7px 12px;border:1px solid #ddd6c8;background:#fbf9f4;font-size:13px;font-family:'Archivo',sans-serif;color:#17130d;border-radius:0;outline:none">
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center;margin-top:8px">
        ...
        <!-- Same treatment as the search field, for the same reason: the price
             row is the same wrapping flex row with the same fixed-width item at
             the end of it. Nobody has reported this one wrapping, but it is the
             identical mechanism and fixing only the field that was noticed
             leaves the bug in place next to it. -->
        <select id="sort" data-mq="sort" style="margin-left:auto;flex:1 1 120px;min-width:120px;max-width:150px;padding:6px 8px;border:1px solid #ddd6c8;background:#fbf9f4;font-size:12.5px;font-family:'Archivo',sans-serif;color:#17130d;border-radius:0;cursor:pointer">
